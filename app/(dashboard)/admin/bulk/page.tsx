...
import { AlertCircle, CheckCircle, Download, FileSpreadsheet, Percent, Upload, Loader2 } from 'lucide-react';
import React, { useState } from 'react';
import * as XLSX from 'xlsx';

export default function BulkOperationsPage() {
    const { restaurant, products, categories, reorderProducts, refreshData } = useMenu();
    ...
    // MIGRATE IMAGES TO FIREBASE
    const [isMigrating, setIsMigrating] = useState(false);
    const handleMigrateImages = async () => {
        if (!confirm("Eski dış linkli (Supabase vb.) tüm görseller Firebase Storage'a yüklenip güncellenecektir. Devam edilsin mi? (Bu işlem ürün sayısına göre biraz zaman alabilir)")) return;

        setIsMigrating(true);
        setImportStatus({ type: 'info', message: 'Görseller taranıyor ve Firebase Storage\'a yükleniyor (Lütfen sekmeyi kapatmayın)...' });
        ...
        try {
            ...
            setImportStatus({
                type: 'success',
                message: `Taşıma işlemi tamamlandı! ${migratedProducts} ürün ve ${migratedCategories} kategori görseli Firebase'e yüklendi.`
            });
        } catch (error: any) {
            setImportStatus({ type: 'error', message: 'Taşıma sırasında hata: ' + error.message });
        } finally {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                {/* Images Migration Card */}
                <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm flex flex-col items-center text-center">
                    <div className="mb-4 rounded-full bg-purple-50 p-4">
                        <Upload className="h-8 w-8 text-purple-600" />
                    </div>
                    <h3 className="text-lg font-bold text-gray-900">Görselleri Taşı</h3>
                    <p className="mt-2 text-sm text-gray-500 mb-4">
                        Eski dış kaynaklı görselleri (Örn: Supabase) Firebase Storage'a yükler ve veritabanını günceller.
                    </p>
                    <div className="w-full mt-auto">
                        <button
                            onClick={handleMigrateImages}
                            disabled={isMigrating}
                            className="w-full rounded-lg bg-purple-600 px-4 py-2 text-sm font-bold text-white hover:bg-purple-700 disabled:opacity-50 flex items-center justify-center gap-2"
                        >
                            {isMigrating ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                            {isMigrating ? 'Taşınıyor...' : 'Firebase\'e Yükle'}
                        </button>
                    </div>
                </div>
            </div>
